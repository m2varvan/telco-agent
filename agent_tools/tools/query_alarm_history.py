...
def execute_query_alarm_history(
    cell_id: str, oss_id: str, year: int, month: int, day: int
) -> dict[str, Any]:
    """
    Synchronous execution function to retrieve downtime counters and alarm records for a cell on a given date.
    """
    ...
    all_alarms = csv_alarms + [a for a in injected if a.get("alarm_id") not in seen]
...
async def query_alarm_history(tool_config: QueryAlarmHistoryConfig, builder: Builder):
    async def _query_alarm_history(
        ...
        return execute_query_alarm_history(cell_id, oss_id, year, month, day)
