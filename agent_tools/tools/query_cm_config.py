...
def execute_query_cm_config(cell_id: str, oss_id: str, before_date: str, days_back: int = 7) -> dict[str, Any]:
    """
    Synchronous execution function to query CM configuration for a cell within a date window.
    """
...
async def query_cm_config(tool_config: QueryCmConfigConfig, builder: Builder):
    async def _query_cm_config(cell_id: str, oss_id: str, before_date: str, days_back: int = 7) -> dict[str, Any]:
        ...
        """
        return execute_query_cm_config(cell_id, oss_id, before_date, days_back)

    yield FunctionInfo.from_fn(
        _query_cm_config,
        description=(
            "Query cm_config_sample.csv for configuration records on a cell within a date window. "
            "Returns changes list with ADMINISTRATIVESTATE, CELLBARRED, DLCHANNELBANDWIDTH, etc. "
